...
def fetch_edu_news(days=14):
    ...
    china_queries = [
        ...
        '(教育部 OR 国家层面) (政策 OR 数字化 OR 民办教育规范)'
    ]

    # --- 国际视野查询 (专项锁定：专业媒体+中国学生专项) ---
    # 分区1：Top100名校针对中国学生的招生政策
    intl_policy_query = '("Top 100 Universities" OR "Ivy League" OR "UCAS") (Admissions for Chinese students OR Visa for China OR international requirements)'
    # 分区2：专业媒体报道的AI教育案例与领袖观点 (EdSurge, EdWeek等)
    intl_ai_query = '(site:edsurge.com OR site:edweek.org OR "UNESCO") (AI classroom practice OR Generative AI Case Study OR Educational Leaders AI perspective)'
        ...
        for entry in feed.entries:
            ...
            if len(all_data[sec_id]["intl"]) < 10:
                ...
                all_data[sec_id]["intl"].append({
                    ...
                    "source": entry.source.get('title', '专业教育媒体'), "date": pub_time.strftime('%m-%d')
                })
        time.sleep(1)
...
def format_html(data, sections):
    ...
    for sec_id, sec_info in sections.items():
        ...
        for reg_id, reg_name in [("china", "📍 中国动态 (京沪杭深/C9/名校)"), ("intl", "🌐 国际视野（TOP100名校留学与招生政策/AI教育实践/）")]:
            ...
            if not items:
                rows += '<tr><td style="padding:15px; color:#94a3b8; font-size:12px; background:#fff; text-align:center;">暂无高度相关的专业资讯</td></tr>'
            else:
                for item in items:
...
def send_email():
    ...
    html = f"""<html><body style="font-family:'PingFang SC',Arial,sans-serif; background:#f8fafc; padding:20px;">
        <div style="max-width:750px; margin:0 auto; background:#fff; border-radius:12px; overflow:hidden; box-shadow:0 10px 30px rgba(0,0,0,0.1); border:1px solid #e2e8f0;">
            <div style="background:#1a365d; padding:35px; text-align:center; color:#fff;">
                ...
                <p style="font-size:13px; margin-top:10px; opacity:0.9;">14天深度观察：EdSurge/EdWeek 专业案例与名校对华政策</p>
    ...
    msg = MIMEMultipart()
    msg['Subject'] = f"垂直教育速递: 14天专业媒体AI案例/名校对华政策 ({datetime.now().strftime('%m/%d')})"
    ...
    print(f"✅ 发送完毕。共计 {total} 条高价值垂直新闻。")
